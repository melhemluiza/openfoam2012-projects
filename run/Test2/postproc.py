...
def get_user_parameters(case_type):
    ...
    try:
        ...
        if case_type == "stagnantB":
            return {"wa0": 0.95, "rho_total": 1.0, "Dab": 0.0000155}
        else:
            return {
                "wa0": 0.8,
                "waL": 0.2,
    ...
    if case_type == "stagnantB":
        ...
        for i, z in enumerate(z_points):
            if z == 0:
                ...
                wa_analytical[i] = 0
                rho_a_analytical[i] = 0
            else:
                rho_a_analytical[i] = rho_total - rho_total / (np.exp(C1 * z - C1))
        ...
        # SOLUÇÃO PARA DIFUSÃO EQUIMOLAR
        wa0 = params["wa0"]
        waL = params["waL"]
        Dab = params["Dab"]
        MA = params["MA"]
        ...
        r = float(MB) / float(MA)
        C2 = (rho_total - (1 - r) * rho_aL) / (rho_total - (1 - r) * rho_a0)
        ...
        for i, z in enumerate(z_points):
            if z == 0:
                ...
                rho_a_analytical[i] = (rho_total / (1 - r)) - (
                    ((rho_total / (1 - r)) - rho_a0) * (C2 ** (z / L))
                )
                wa_analytical[i] = rho_a_analytical[i] / rho_total
