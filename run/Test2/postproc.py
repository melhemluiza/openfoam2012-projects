...
def calculate_analytical_wb_stagnantB(
    z_points, rho_a0, rho_aL, Dab, MA, MB, rho_total, L=1.0
):
...
def calculate_analytical_ja_stagnantB(
    ...
    # Calcular gradiente numérico de rho_a
    grad_rho_a = np.gradient(rho_a_analytical, z_points)
    return -Dab * grad_rho_a
...
def calculate_analytical_jb_stagnantB(
    ...
    # jb = -Dab * grad(rho_b)
    rho_b_analytical = calculate_analytical_rho_b_stagnantB(
        z_points, rho_a0, rho_aL, Dab, MA, MB, rho_total, L
    )
    # Calcular gradiente numérico de rho_b
    grad_rho_b = np.gradient(rho_b_analytical, z_points)
    return -Dab * grad_rho_b
...
def calculate_analytical_U_stagnantB(
    ...
    # U = ja / rho_b (definição atualizada)
    ja_analytical = calculate_analytical_ja_stagnantB(
        z_points, rho_a0, rho_aL, Dab, MA, MB, rho_total, L
    )
    rho_b_analytical = calculate_analytical_rho_b_stagnantB(
        z_points, rho_a0, rho_aL, Dab, MA, MB, rho_total, L
    ...
    epsilon = 1e-12
    rho_b_safe = np.where(np.abs(rho_b_analytical) < epsilon, epsilon, rho_b_analytical)

    return ja_analytical / rho_b_safe


def calculate_analytical_U_ver_stagnantB(
    z_points, rho_a0, rho_aL, Dab, MA, MB, rho_total, L=1.0
):
    # U_ver = (Na + Nb) / rho_total
    ...
    Nb_analytical = calculate_analytical_Nb_stagnantB(
        z_points, rho_a0, rho_aL, Dab, MA, MB, rho_total, L
    )
    return (Na_analytical + Nb_analytical) / rho_total
...
def plot_and_export_openfoam_data(
    ...
    if os.path.exists(vector_file):
        ...
        try:
            ...
            if df_vector.shape[1] >= 10:
                ...
                all_data["U"] = df_vector.iloc[:, 9]  # Coluna 9 = U
        ...
        print("Aviso: Arquivo Na_Nb_U.xy não encontrado")
    ...
    # Imediatamente antes da seção "Calcular soluções analíticas"
    ...
    if case_type == "equimolar":
        ...
        all_data["wb_analytical"] = calculate_analytical_wb_stagnantB(
            all_data["z"].values, rho_a0, rho_aL, Dab, MA, MB, rho_total
        )
        all_data["U_analytical"] = calculate_analytical_U_stagnantB(
            all_data["z"].values, rho_a0, rho_aL, Dab, MA, MB, rho_total
        )
        all_data["U_ver_analytical"] = calculate_analytical_U_ver_stagnantB(
            all_data["z"].values, rho_a0, rho_aL, Dab, MA, MB, rho_total
        )
    ...
    # Definir colunas para exportar - REMOVIDOS CAMPOS DESNECESSÁRIOS
    cols_to_export = ["z"]
...
def plot_from_csv(
    ...
    # Ler dados do CSV - CORREÇÃO: usar os mesmos parâmetros de escrita
    try:
        ...
        # Tentar ler de forma alternativa
        try:
            data = pd.read_csv(csv_file, sep=";")
            print("Lido com sep=';' apenas")
        except:
            try:
                data = pd.read_csv(csv_file)
                print("Lido com parâmetros padrão")
    ...
    if "wa" in data.columns and "wa_analytical" in data.columns:
        plt.figure(figsize=(10, 7))
        plt.plot(
            data["z"],
            data["wa"],
        ...
        if "wb" in data.columns and "wb_analytical" in data.columns:
            plt.plot(
                data["z"],
                data["wb"],
                "v",
        ...
        plt.title(
            f"Comparação de Frações Mássicas - Caso: {case_type}\nρ_a0={rho_a0}, ρ_aL={rho_aL}"
        )
        plt.legend()
        plt.grid(True, alpha=0.4)
        plt.xlim(0, 1)
    ...
    if (
        ...
        ax1.set_title(f"Comparação de Velocidades - Caso: {case_type}")
        ax1.legend()
    ...
    if "erro_absoluto_wa" in data.columns:
        print(f"wa - Erro Médio Absoluto: {data['erro_absoluto_wa'].abs().mean():.6e}")
    if "erro_absoluto_wb" in data.columns:
    ...
    if "erro_absoluto_U_ver" in data.columns:
        print(
