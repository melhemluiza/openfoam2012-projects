...
def main():
    ...
    # Arquivos específicos - REMOVIDO REFERÊNCIA AO ARQUIVO COM U
    cloud1_file = os.path.join(latest_time_dir, "myCloud_rho_rho_a_rho_b_wa_wb.xy")
    cloud2_file = os.path.join(
        latest_time_dir, "myCloud_Na_Nb_U_ver_ja_jb.xy"
    )  # Nome corrigido
    ...
    # Ler o segundo arquivo (Na, Nb, U_ver, ja, jb) - 16 colunas (sem U)
    df_cloud2 = read_openfoam_data(cloud2_file)
    if df_cloud2 is None:
        sys.exit(1)

    # Estrutura para 16 colunas (sem U)
    if df_cloud2.shape[1] == 16:
        df_cloud2.columns = [
            "z",
            "x",
            "y",
        ...
        print("✅ Colunas do arquivo 2 identificadas (16 colunas - sem U)")
    else:
    ...
    # Como não temos U, calculamos ja e jb diretamente da definição
    if "ja" not in data_combined.columns:
        # Para calcular ja precisamos de U, que não temos
        # Vamos usar a relação: ja = Na - rho_a * U, mas U = U_ver
        if (
            "U_ver" in data_combined.columns
            and "rho_a" in data_combined.columns
            and "Na" in data_combined.columns
        ):
            data_combined["ja"] = (
                data_combined["Na"] - data_combined["rho_a"] * data_combined["U_ver"]
            )
        else:
            print(
                "⚠️  Não foi possível calcular ja - campos necessários não disponíveis"
            )

    if "jb" not in data_combined.columns:
        if (
            "U_ver" in data_combined.columns
            ...
            data_combined["jb"] = (
                data_combined["Nb"] - data_combined["rho_b"] * data_combined["U_ver"]
            )
        else:
            print(
                "⚠️  Não foi possível calcular jb - campos necessários não disponíveis"
            )
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Campos disponíveis: rho_a, rho_b, wa, wb, ja, jb, Na, Nb, U_ver"
        )  # Removido U
        sys.exit(1)
